...
class QuantLinearLoRA(nn.Module):
    ...
    def __init__(self, bits, groupsize, infeatures, outfeatures, bias, r=8, lora_alpha=16, lora_dropout=0.05):
        super().__init__()
        ...
        if bias:
            ...
            self.bias = None
        self.equip_with_lora(r, infeatures, outfeatures, lora_alpha, lora_dropout) # this can be excute out of the class if necessary
        
    def equip_with_lora(self, r, infeatures, outfeatures, lora_alpha, lora_dropout):
        ####### lora here
        if r > 0:
            self.lora_A = nn.Linear(infeatures, r, bias=False)
            self.lora_B = nn.Linear(r, outfeatures, bias=False)
...
def make_quant_linear(module, names, bits, groupsize, name=''):
    ...
    for attr in dir(module):
        ...
        if name1 in names:
            delattr(module, attr)
            setattr(module, attr, QuantLinearLoRA(bits, groupsize, tmp.in_features, tmp.out_features, tmp.bias is not None))
...
def autotune_warmup_linear(model, transpose=False):
    ...
    for _, m in model.named_modules():
        if not isinstance(m, QuantLinearLoRA):
            continue
